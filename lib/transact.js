	...
	Transact.prototype.between = function( v1,v2 ) {
		if (this.pending.if !== null) {
			...
			if (this.pending.if.not === true) {
				...
				var _v1 = this._valuefy( this.pending.if.attr, ':ifnotbtwlo_',  util.stringify(v1) )
				var _v2 = this._valuefy( this.pending.if.attr, ':ifnotbtwhi_',  util.stringify(v2) )
				this.pending.ConditionExpression.push(
					"(NOT ( " + _name + " BETWEEN " + _v1 + " AND " + _v2 + " ))"
				)
			} else {
				var _name = this._namefy( this.pending.if.attr, '#ifbtw_' )
				var _v1 = this._valuefy( this.pending.if.attr, ':ifbtwlo_',  util.stringify(v1) )
				var _v2 = this._valuefy( this.pending.if.attr, ':ifbtwhi_',  util.stringify(v2) )
				this.pending.ConditionExpression.push(
		...
		if (this.pending.if !== null) {
			...
			if (this.pending.if.not === true) {
				...
				value.map(function(v,idx) {
					var _vname = $this._valuefy( $this.pending.if.attr, ':ifnotin' + idx + '_',  util.stringify(v) )
				...
				var ins = []
				value.map(function(v,idx) {
					var _vname = $this._valuefy( $this.pending.if.attr, ':ifin' + idx + '_',  util.stringify(v) )
	...
	Transact.prototype.begins_with = function( value ) {
		if (this.pending.if !== null) {
			...
			if (this.pending.if.not === true) {
				...
				var _vname = this._valuefy( this.pending.if.attr, ':ifnotbw_',  util.stringify(value) )
				...
				var _vname = this._valuefy( this.pending.if.attr, ':ifbw_',  util.stringify(value) )
