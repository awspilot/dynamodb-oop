...
var util = require('@awspilot/dynamodb-util')
	...
	Transact.prototype.eq = function( value ) {
		...
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#if_'+ this.pending.if.attr ] = this.pending.if.attr;
			this.pending.ExpressionAttributeValues[ ':if_'+ this.pending.if.attr ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"( #if_" + this.pending.if.attr + " = :if_" + this.pending.if.attr  + ")"
			)
	...
	Transact.prototype.exists = function( value ) {
		if (this.pending.if !== null) {
			...
			if (this.pending.if.not === true) {
				this.pending.ExpressionAttributeNames [ '#ifnotexists_'+ this.pending.if.attr ] = this.pending.if.attr;
				this.pending.ConditionExpression.push(
					"attribute_not_exists( #ifnotexists_" + this.pending.if.attr + ")"
				)
			} else {
				this.pending.ExpressionAttributeNames [ '#ifexists_'+ this.pending.if.attr ] = this.pending.if.attr;
				this.pending.ConditionExpression.push(
					"attribute_exists( #ifexists_" + this.pending.if.attr + ")"
				)
			}
			this.pending.if = null;
			//this.debug()
	...
	Transact.prototype.ne = function( value ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#ifne_'+ this.pending.if.attr ] = this.pending.if.attr;
			this.pending.ExpressionAttributeValues[ ':ifne_'+ this.pending.if.attr ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"( #ifne_" + this.pending.if.attr + " <> :ifne_" + this.pending.if.attr  + ")"
			)
			this.pending.if = null;
			return this
		}
	...
	Transact.prototype.ge = function( value ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#ifge_'+ this.pending.if.attr ] = this.pending.if.attr;
			this.pending.ExpressionAttributeValues[ ':ifge_'+ this.pending.if.attr ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"( #ifge_" + this.pending.if.attr + " >= :ifge_" + this.pending.if.attr  + ")"
			)
			this.pending.if = null;
			return this
		}
	...
	Transact.prototype.gt = function( value ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#ifgt_'+ this.pending.if.attr ] = this.pending.if.attr;
			this.pending.ExpressionAttributeValues[ ':ifgt_'+ this.pending.if.attr ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"( #ifgt_" + this.pending.if.attr + " > :ifgt_" + this.pending.if.attr  + ")"
			)
			this.pending.if = null;
			return this
		}
	...
	Transact.prototype.le = function( value ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#ifle_'+ this.pending.if.attr ] = this.pending.if.attr;
			this.pending.ExpressionAttributeValues[ ':ifle_'+ this.pending.if.attr ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"( #ifle_" + this.pending.if.attr + " <= :ifle_" + this.pending.if.attr  + ")"
			)
			this.pending.if = null;
			return this
		}
	...
	Transact.prototype.lt = function( value ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#iflt_'+ this.pending.if.attr ] = this.pending.if.attr;
			this.pending.ExpressionAttributeValues[ ':iflt_'+ this.pending.if.attr ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"( #iflt_" + this.pending.if.attr + " < :iflt_" + this.pending.if.attr  + ")"
			)
			this.pending.if = null;
			return this
		}
	...
	Transact.prototype.between = function( v1,v2 ) {
		if (this.pending.if !== null) {
			...
			if (this.pending.if.not === true) {
				this.pending.ExpressionAttributeNames [ '#ifnotbtw_'+ this.pending.if.attr ] = this.pending.if.attr;
				this.pending.ExpressionAttributeValues[ ':ifnotbtwlo_'+ this.pending.if.attr ] = util.stringify(v1)
				this.pending.ExpressionAttributeValues[ ':ifnotbtwhi_'+ this.pending.if.attr ] = util.stringify(v2)
				this.pending.ConditionExpression.push(
					"(NOT ( #ifnotbtw_" + this.pending.if.attr + " BETWEEN :ifnotbtwlo_" + this.pending.if.attr  + " AND :ifnotbtwhi_" + this.pending.if.attr  + "))"
				)
			} else {
				this.pending.ExpressionAttributeNames [ '#ifbtw_'+ this.pending.if.attr ] = this.pending.if.attr;
				this.pending.ExpressionAttributeValues[ ':ifbtwlo_'+ this.pending.if.attr ] = util.stringify(v1)
				this.pending.ExpressionAttributeValues[ ':ifbtwhi_'+ this.pending.if.attr ] = util.stringify(v2)
				this.pending.ConditionExpression.push(
					"( #ifbtw_" + this.pending.if.attr + " BETWEEN :ifbtwlo_" + this.pending.if.attr  + " AND :ifbtwhi_" + this.pending.if.attr  + ")"
				)
			}
			this.pending.if = null;
	...
	Transact.prototype.in = function( value ) {
		...
		if (this.pending.if !== null) {
			...
			if (this.pending.if.not === true) {
				this.pending.ExpressionAttributeNames [ '#ifnotin_'+ this.pending.if.attr ] = this.pending.if.attr;

				var ins = []
				value.map(function(v,idx) {
					$this.pending.ExpressionAttributeValues[ ':ifnotin' + idx + '_' + $this.pending.if.attr ] = util.stringify(v)
					ins.push( ':ifnotin' + idx + '_' + $this.pending.if.attr )
				})

				this.pending.ConditionExpression.push(
					"(NOT ( #ifnotin_" + this.pending.if.attr + " IN (" + ins.join(' , ') + ") ))"
				)
			} else {
				this.pending.ExpressionAttributeNames [ '#ifin_'+ this.pending.if.attr ] = this.pending.if.attr;

				var ins = []
				value.map(function(v,idx) {
					$this.pending.ExpressionAttributeValues[ ':ifin' + idx + '_' + $this.pending.if.attr ] = util.stringify(v)
					ins.push( ':ifin' + idx + '_' + $this.pending.if.attr )
				})

				this.pending.ConditionExpression.push(
					"( #ifin_" + this.pending.if.attr + " IN (" + ins.join(' , ') + ") )"
				)
			}
	...
	Transact.prototype.contains = function( value ) {
		if (this.pending.if !== null) {
			...
			if (this.pending.if.not === true) {
				this.pending.ExpressionAttributeNames [ '#ifnotcontains_'+ this.pending.if.attr ] = this.pending.if.attr;
				this.pending.ExpressionAttributeValues[ ':ifnotcontains_'+ this.pending.if.attr ] = util.stringify(value)
				this.pending.ConditionExpression.push(
					"( NOT contains( #ifnotcontains_" + this.pending.if.attr + " , :ifnotcontains_" + this.pending.if.attr  + ") )"
				)
			} else {
				this.pending.ExpressionAttributeNames [ '#ifcontains_'+ this.pending.if.attr ] = this.pending.if.attr;
				this.pending.ExpressionAttributeValues[ ':ifcontains_'+ this.pending.if.attr ] = util.stringify(value)
				this.pending.ConditionExpression.push(
					"contains( #ifcontains_" + this.pending.if.attr + " , :ifcontains_" + this.pending.if.attr  + ")"
				)
			}
			this.pending.if = null;
	...
	Transact.prototype.begins_with = function( value ) {
		if (this.pending.if !== null) {
			...
			if (this.pending.if.not === true) {
				this.pending.ExpressionAttributeNames [ '#ifnotbw_'+ this.pending.if.attr ] = this.pending.if.attr;
				this.pending.ExpressionAttributeValues[ ':ifnotbw_'+ this.pending.if.attr ] = util.stringify(value)
				this.pending.ConditionExpression.push(
					"(NOT begins_with( #ifnotbw_" + this.pending.if.attr + " , :ifnotbw_" + this.pending.if.attr  + "))"
				)
			} else {
				this.pending.ExpressionAttributeNames [ '#ifbw_'+ this.pending.if.attr ] = this.pending.if.attr;
				this.pending.ExpressionAttributeValues[ ':ifbw_'+ this.pending.if.attr ] = util.stringify(value)
				this.pending.ConditionExpression.push(
					"begins_with( #ifbw_" + this.pending.if.attr + " , :ifbw_" + this.pending.if.attr  + ")"
				)
			}
			this.pending.if = null;
