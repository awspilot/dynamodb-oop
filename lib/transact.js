...
var util = require('@awspilot/dynamodb-util')
	...
	Transact.prototype.ne = function( value ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#ifne_'+ this.pending.if ] = this.pending.if;
			this.pending.ExpressionAttributeValues[ ':ifne_'+ this.pending.if ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"( #ifne_" + this.pending.if + " <> :ifne_" + this.pending.if  + ")"
			)
			this.pending.if = null;
			return this
		}
	...
	Transact.prototype.ge = function( value ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#ifge_'+ this.pending.if ] = this.pending.if;
			this.pending.ExpressionAttributeValues[ ':ifge_'+ this.pending.if ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"( #ifge_" + this.pending.if + " >= :ifge_" + this.pending.if  + ")"
			)
			this.pending.if = null;
			return this
		}
	...
	Transact.prototype.gt = function( value ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#ifgt_'+ this.pending.if ] = this.pending.if;
			this.pending.ExpressionAttributeValues[ ':ifgt_'+ this.pending.if ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"( #ifgt_" + this.pending.if + " > :ifgt_" + this.pending.if  + ")"
			)
			this.pending.if = null;
			return this
		}
	...
	Transact.prototype.le = function( value ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#ifle_'+ this.pending.if ] = this.pending.if;
			this.pending.ExpressionAttributeValues[ ':ifle_'+ this.pending.if ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"( #ifle_" + this.pending.if + " <= :ifle_" + this.pending.if  + ")"
			)
			this.pending.if = null;
			return this
		}
	...
	Transact.prototype.lt = function( value ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#iflt_'+ this.pending.if ] = this.pending.if;
			this.pending.ExpressionAttributeValues[ ':iflt_'+ this.pending.if ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"( #iflt_" + this.pending.if + " < :iflt_" + this.pending.if  + ")"
			)
			this.pending.if = null;
			return this
		}
	...
	Transact.prototype.between = function( v1,v2 ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#ifbtw_'+ this.pending.if ] = this.pending.if;
			this.pending.ExpressionAttributeValues[ ':ifbtwlo_'+ this.pending.if ] = util.stringify(v1)
			this.pending.ExpressionAttributeValues[ ':ifbtwhi_'+ this.pending.if ] = util.stringify(v2)
			this.pending.ConditionExpression.push(
				"( #ifbtw_" + this.pending.if + " BETWEEN :ifbtwlo_" + this.pending.if  + " AND :ifbtwhi_" + this.pending.if  + ")"
			)
			this.pending.if = null;
			return this
		}
	...
	Transact.prototype.in = function( value ) {
		...
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#ifin_'+ this.pending.if ] = this.pending.if;

			var ins = []
			value.map(function(v,idx) {
				$this.pending.ExpressionAttributeValues[ ':ifin' + idx + '_' + $this.pending.if ] = util.stringify(v)
				ins.push( ':ifin' + idx + '_' + $this.pending.if )
			})

			this.pending.ConditionExpression.push(
				"( #ifin_" + this.pending.if + " IN (" + ins.join(' , ') + ") )"
			)
			this.pending.if = null;
	...
	Transact.prototype.contains = function( value ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#ifcontains_'+ this.pending.if ] = this.pending.if;
			this.pending.ExpressionAttributeValues[ ':ifcontains_'+ this.pending.if ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"contains( #ifcontains_" + this.pending.if + " , :ifcontains_" + this.pending.if  + ")"
			)
			this.pending.if = null;
	...
	Transact.prototype.not_contains = function( value ) {
		if (this.pending.if !== null) {

			this.pending.ExpressionAttributeNames [ '#ifnotcontains_'+ this.pending.if ] = this.pending.if;
			this.pending.ExpressionAttributeValues[ ':ifnotcontains_'+ this.pending.if ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"( NOT contains( #ifnotcontains_" + this.pending.if + " , :ifnotcontains_" + this.pending.if  + ") )"
			)
			this.pending.if = null;
	...
	Transact.prototype.begins_with = function( value ) {
		if (this.pending.if !== null) {
			...
			this.pending.ExpressionAttributeNames [ '#ifbw_'+ this.pending.if ] = this.pending.if;
			this.pending.ExpressionAttributeValues[ ':ifbw_'+ this.pending.if ] = util.stringify(value)
			this.pending.ConditionExpression.push(
				"begins_with( #ifbw_" + this.pending.if + " , :ifbw_" + this.pending.if  + ")"
			)
			this.pending.if = null;
