...
var util = require('@awspilot/dynamodb-util')
	...
	Transact.prototype.insert_or_update = function( item ) {
		...
		if ( SET_UpdateExpression.length || REMOVE_UpdateExpression.length ) {
			...
			if ( SET_UpdateExpression.length )
				$thisQuery.Update.UpdateExpression += "SET " + SET_UpdateExpression.join(' , ') + ' ';

			if ( REMOVE_UpdateExpression.length )
				$thisQuery.Update.UpdateExpression += "REMOVE " + REMOVE_UpdateExpression.join(' , ') + ' ';
	...
	Transact.prototype.update = function( item ) {
		...
		if (Object.keys(this.pending.ExpressionAttributeValues).length)
			$thisQuery.Update.ExpressionAttributeValues = this.pending.ExpressionAttributeValues;
