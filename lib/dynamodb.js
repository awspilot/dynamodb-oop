...
var $events = {
	...
	DynamoDB.prototype.table = function($tableName) {
		return new Request( this.client ).table($tableName)
	}
	...
	Request.prototype.insert = function(item, callback) {
		...
		if (Object.keys(this.ifFilter).length > 0) {
			...
			this.client.putItem( $thisQuery, function(err,data) {
				if (err) {
			...
			this.client.describeTable( $thisQuery,function(err,data) {
				if (err) {
	...
	Request.prototype.update = function($attrz, callback, $action ) {
		...
		this.client.describeTable( $lastQuery,function(err,data) {
			if (err) {
			...
			$this.client.updateItem( $thisQuery, function(err,data) {
				if (err) {
	...
	Request.prototype.delete = function($attrz, callback ) {
		...
		if (typeof $attrz == 'function') {
			...
			this.client.deleteItem( $thisQuery, function(err,data) {
				if (err) {
	...
	Request.prototype.get = function(callback) {
		...
		this.client.getItem( $thisQuery, function(err,data) {
			if (err) {
	...
	Request.prototype.query = function(callback) {
		...
		this.client.query( $thisQuery , function(err,data) {
			if (err) {
	...
	Request.prototype.scan = function( callback ) {
		...
		this.client.scan( $thisQuery , function(err,data) {
			if (err) {
