...
// @todo: nice handling of throtteling https://github.com/aws/aws-sdk-js/issues/402
	...
	Request.prototype.registerExpressionAttributeName = function(item) {
		...
		return item.split('.').map(function(original_attName) {

			var attName =  original_attName.split('-').join('_minus_') // "-" not allowed
	...
	Request.prototype.registerExpressionAttributeValue = function(name, value) {
		...
		name = name.split('-').join('_minus_') // "-" not allowed
	...
	Request.prototype.buildKeyConditionExpression = function(idx) {
		...
		this.KeyConditionExpression = Object.keys(this.whereKey).map(function(key) {
			return $this.registerExpressionAttributeName(key) + ' ' +
				'=' + ' ' +
				$this.registerExpressionAttributeValue(key, util.normalizeItem({value: $this.whereKey[key] }).value )
		}).concat(
				...
				switch (filterOperators[whereFilter.operator]) {
					...
					case '>=':
						return $this.registerExpressionAttributeName(key) + ' ' +
							...
							$this.registerExpressionAttributeValue(key, whereFilter.value)
						break

					case  'BETWEEN':
						return $this.registerExpressionAttributeName(key) + ' BETWEEN ' +
							$this.registerExpressionAttributeValue(key+'_1', whereFilter.value) +
							' AND ' +
							$this.registerExpressionAttributeValue(key+'_2', whereFilter.value2)
						break;

					case 'begins_with':
						return 'begins_with(' + $this.registerExpressionAttributeName(key) + ', ' + $this.registerExpressionAttributeValue(key, whereFilter.value) + ')'
						break;
