...
// @todo: nice handling of throtteling https://github.com/aws/aws-sdk-js/issues/402
	...
	DynamoDB.prototype.add = function(data, datatype ) {
		...
		if (typeof datatype == "string") {
			switch (datatype) {
				case 'N':
					return this.add(this.N(data))
					break
				case 'NS':
					return this.add(this.NS(data))
					break
				case 'SS':
					return this.add(this.SS(data))
					break;
	...
	DynamoDB.prototype.del = function(data, datatype) {
		...
		if (Array.isArray(data)) {
			...
			for (var i in data) {
				switch (datatype) {
					case 'L':
					to_ret[datatype][i] = util.anormalizeValue( data[i] )
					break;
					case 'SS':
					to_ret[datatype][i] = data[i].toString()
					break;
					case 'NS':
					to_ret[datatype][i] = data[i].toString()
					break;
	...
	function Request( $client, $events ) {
		...
		this.ReturnConsumedCapacity = DynamoDB.INDEXES
