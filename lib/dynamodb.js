...
// @todo: nice handling of throtteling https://github.com/aws/aws-sdk-js/issues/402
	...
	Request.prototype.buildConditionExpression = function(idx) {
		var $this = this

		if (!this.ifConditionExpression.length)
			return

		var ret = []
		this.ConditionExpression = this.ifConditionExpression.map(function(ifCondition) {
			//var key = ifCondition.attribute

			//switch (filterOperators[ifCondition.operator]) {
			//	case '=':
			//	case '<>':
			//	case '<':
			//	case '<=':
			//	case '>':
			//	case '>=':
			//		return $this.registerExpressionAttributeName(ifCondition.attribute) + ' ' +
			//			filterOperators[ifCondition.operator] + ' ' +
			//			$this.registerExpressionAttributeValue(ifCondition.attribute, ifCondition.value)
			//		break

			//	case  'BETWEEN':
			//		return $this.registerExpressionAttributeName(ifCondition.attribute) + ' BETWEEN ' +
			//			$this.registerExpressionAttributeValue(ifCondition.attribute+'_1', ifCondition.value) +
			//			' AND ' +
			//			$this.registerExpressionAttributeValue(ifCondition.attribute+'_2', ifCondition.value2)
			//		break;

			//	case 'IN':
			//		return $this.registerExpressionAttributeName(ifCondition.attribute) + ' IN (' +
			//				ifCondition.value.map(function(v, idx) {
			//					return $this.registerExpressionAttributeValue(ifCondition.attribute+'_' + idx, v)
			//				}).join(',')  +
			//			' )'
			//		break;
			...
			//	case 'attribute_exists':
			//		return 'attribute_exists(' + $this.registerExpressionAttributeName(ifCondition.attribute) + ')'
			//		break;

			//	case 'attribute_not_exists':
			//		return 'attribute_not_exists(' + $this.registerExpressionAttributeName(ifCondition.attribute) + ')'
			//		break;

			//	case 'begins_with':
			//		return 'begins_with(' + $this.registerExpressionAttributeName(ifCondition.attribute) + ', ' + $this.registerExpressionAttributeValue(ifCondition.attribute, ifCondition.value) + ')'
			//		break;

			//	case 'contains':
			//		return 'contains(' + $this.registerExpressionAttributeName(ifCondition.attribute) + ', ' + $this.registerExpressionAttributeValue(ifCondition.attribute, ifCondition.value) + ')'
			//		break;

			//	case 'not_contains':
			//		return 'NOT contains(' + $this.registerExpressionAttributeName(ifCondition.attribute) + ', ' + $this.registerExpressionAttributeValue(ifCondition.attribute, ifCondition.value) + ')'
			//		break;
			//	//attribute_type (path, type)
			//	//size (path)
			//}
