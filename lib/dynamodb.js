'use strict';
			...
			if (this.whereOther.hasOwnProperty(key)) {
					...
					if (this.whereOther[key].hasOwnProperty('value2') && this.whereOther[key].value2 !== undefined ) {
						anormal[key] = {
							...
							AttributeValueList: [ util.stringify( this.whereOther[key].value ), util.stringify( this.whereOther[key].value2 ) ]
						}
					} else {
						anormal[key] = {
							ComparisonOperator: this.whereOther[key].operator,
							AttributeValueList: [ util.stringify( this.whereOther[key].value ) ]
						}
					}
			}
		}
	...
	// RAW functions
	Request.prototype.RawIndexName = function( $IndexName ) {
		this.IndexName = $IndexName
		return this
	}
