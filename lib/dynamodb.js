...
function DynamoDB() {
	...
	this.insert = function(item, callback) {
		...
		if (Object.keys(this.ifFilter).length > 0) {
			...
			var $thisQuery = {
				...
				Expected: this.buildExpected( this.ifFilter ),
	...
	this.compare = function( $comparison, $value , $value2 ) {
		...
		if (this.pendingIf !== null) {
			this.ifFilter[this.pendingIf] = { operator: $comparison, type: this.anormalizeType($value), value: $value, value2: $value2 }
			this.pendingIf = null
			return this
		}
	...
	this.buildExpected = function( $expected ) {
		var anormal = {}
			...
			if ($expected.hasOwnProperty(key)) {
					...
					if ($expected[key].hasOwnProperty('value2') && $expected[key].value2 !== undefined ) {
						anormal[key] = {
							...
							AttributeValueList: [ this.anormalizeValue( $expected[key].value ), this.anormalizeValue( $expected[key].value2 ) ]	
						}
					} else {
						anormal[key] = {
							ComparisonOperator: $expected[key].operator,
							AttributeValueList: [ this.anormalizeValue( $expected[key].value ) ]	
						}
					}
			}
		}
